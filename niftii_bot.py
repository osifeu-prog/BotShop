...
if not TOKEN:
    ...
    print("💡 אנא הרץ שוב את setup.bat והזן טוקן תקין")
    sys.exit(1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    info_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("📖 מידע", callback_data="open_shop")
    ]])
...
async def receive_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.photo:
        ...
        await update.message.reply_text(
            "❌ אנא העלה תמונה של אישור ההעברה.",
            reply_markup=BANK_TRANSFER_KB
        )
        return PAYMENT_CONFIRMATION
...
def main():
    ...
    # מטפלי הודעות טקסט - תיקון הכפתור
