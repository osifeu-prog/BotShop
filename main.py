...
from slh_core_api import router as slh_core_router
...
def payment_links_keyboard() -> InlineKeyboardMarkup:
    ...
    # BIT_URL יכול להיות גם מספר טלפון (לא URL) – במקרה כזה לא נוסיף כפתור שבור
    if BIT_URL and (BIT_URL.startswith("http://") or BIT_URL.startswith("https://")):
...
async def payment_method_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    bit_extra = ""
    if BIT_URL and not (BIT_URL.startswith("http://") or BIT_URL.startswith("https://")):
        # במקרה של מספר טלפון – נוסיף לטקסט במקום כפתור לא חוקי
        bit_extra = f"\nמספר ביט לתשלום: {BIT_URL}\n"

    text = (
        f"{details_text}{bit_extra}\n"
        "לאחר ביצוע התשלום:\n"
        "1. שלח כאן *צילום מסך או תמונה* של אישור התשלום.\n"
        "2. הבוט יעביר את האישור למארגנים לבדיקה.\n"
        "3. לאחר אישור ידני תקבל קישור לקהילת העסקים.\n"
