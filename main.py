...
I18N_MESSAGES: Dict[str, Dict[str, str]] = {
    "start_message": {
        ...
        "ru": (
            ...
            "• Доступ в общее игровое/комьюнити-сообщество\n"
    ...
    "btn_main_join": {
        "he": "🚀 הצטרפות לקהילת העסקים (39 ₪)",
        "en": "🚀 Join the business community (39 ₪)",
        "ar": "🚀 انضم إلى مجتمع الأعمال (39 ₪)",
        "ru": "🚀 Присоединиться к бизнес-сообществу (39 ₪)",
    },
    "btn_main_asset_info": {
        "he": "💎 מה זה הנכס הדיגיטלי?",
        "en": "💎 What is the digital asset?",
        "ar": "💎 ما هو الأصل الرقمي؟",
        "ru": "💎 Что такое цифровой актив?",
    },
    "btn_main_share": {
        "he": "🔗 שתף את שער הקהילה",
        "en": "🔗 Share the community gate",
        "ar": "🔗 شارك بوابة المجتمع",
        "ru": "🔗 Поделиться входом в сообщество",
    },
    "btn_main_vision": {
        "he": "🌟 חזון SLH",
        "en": "🌟 SLH Vision",
    ...
    "btn_pay_paybox": {
        "he": "📲 ביט / פייבוקס / PayPal",
        "en": "📲 Bit / PayBox / PayPal",
        "ar": "📲 Bit / PayBox / PayPal",
        "ru": "📲 Bit / PayBox / PayPal",
    },
    "btn_pay_ton": {
        "he": "💎 טלגרם (TON)",
        "en": "💎 Telegram (TON)",
    ...
    "btn_set_bank": {
        "he": "🏦 הגדר פרטי בנק",
        "en": "🏦 Set bank details",
        "ar": "🏦 إعداد تفاصيل البنك",
        "ru": "🏦 Настроить банковские реквизиты",
    },
    "btn_set_groups": {
        "he": "👥 הגדר קבוצות",
        "en": "👥 Set groups",
    ...
    "btn_show_asset": {
        "he": "📊 הצג נכס דיגיטלי",
        "en": "📊 Show digital asset",
        "ar": "📊 عرض الأصل الرقمي",
        "ru": "📊 Показать цифровой актив",
    },
    "btn_support_group": {
        "he": "קבוצת תמיכה",
        "en": "Support group",
...
def get_lang_for_update(update: Update) -> str:
    """קובע שפה למשתמש: קודם מה-DB, אם אין – מה-Telegram, אחרת ברירת מחדל."""
...
def main_menu_keyboard(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(t("btn_main_join", lang), callback_data="join"),
        ],
        [
            InlineKeyboardButton(t("btn_main_asset_info", lang), callback_data="digital_asset_info"),
        ],
        [
            InlineKeyboardButton(t("btn_main_share", lang), callback_data="share"),
        ],
        [
            InlineKeyboardButton(t("btn_main_vision", lang), callback_data="vision"),
        ],
        [
            InlineKeyboardButton(t("btn_main_my_area", lang), callback_data="my_area"),
        ],
        [
            InlineKeyboardButton(t("btn_main_support", lang), callback_data="support"),
        ],
    ])

def payment_methods_keyboard(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(t("btn_pay_bank", lang), callback_data="pay_bank"),
        ],
        [
            InlineKeyboardButton(t("btn_pay_paybox", lang), callback_data="pay_paybox"),
        ],
        [
            InlineKeyboardButton(t("btn_pay_ton", lang), callback_data="pay_ton"),
        ],
        [
...
def payment_links_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton("פתח קישור התשלום (PayBox / Bit / PayPal)", url=PAYBOX_URL),
        ],
    ]
    if LANDING_URL:
        buttons.append(
            [
                InlineKeyboardButton("דף הנחיתה / הסבר מלא", url=LANDING_URL),
            ]
        )
    buttons.append(
        [
...
def my_area_keyboard(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(t("btn_set_bank", lang), callback_data="set_bank"),
        ],
        [
            InlineKeyboardButton(t("btn_set_groups", lang), callback_data="set_groups"),
        ],
        [
            InlineKeyboardButton(t("btn_show_asset", lang), callback_data="show_asset"),
        ],
        [
            InlineKeyboardButton(t("btn_back", lang), callback_data="back_main"),
        ],
...
def support_keyboard(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(t("btn_support_group", lang), url=SUPPORT_GROUP_LINK),
        ],
        [
            InlineKeyboardButton(t("btn_contact_dev", lang), url=f"tg://user?id={DEVELOPER_USER_ID}"),
        ],
        [
            InlineKeyboardButton(t("btn_back", lang), callback_data="back_main"),
        ],
...
async def digital_asset_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        text,
        parse_mode="Markdown",
        reply_markup=main_menu_keyboard(get_lang_for_update(update)),
    )
...
async def join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        text,
        parse_mode="Markdown",
        reply_markup=payment_methods_keyboard(get_lang_for_update(update)),
    )
...
async def my_area_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        text,
        parse_mode="Markdown",
        reply_markup=my_area_keyboard(get_lang_for_update(update)),
    )
...
async def support_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        text,
        parse_mode="Markdown",
        reply_markup=support_keyboard(get_lang_for_update(update)),
    )
